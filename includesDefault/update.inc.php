...
elseif (isset($_POST['btn_updateUser'])){
    $userID = $_POST['userID'];
    ...
    try {
        $prof = new DefaultContr();
        $prof->updateStudentProfile($name, $surname, $secondName, $studentID, $nationalID, $sex, $country, $phone, $email, $address, $dob, $userID);
    } catch (TypeError $e) {
        echo "Error" . $e->getMessage();
    }
}

elseif (isset($_POST['btn_update_isStillStudent'])){
    $userID = $_POST['userID'];
    $isStillStudent = $_POST['isStillStudent'];
    try {
        $prof = new DefaultContr();
        $prof->updateisStillStudent($isStillStudent, $userID);
    } catch (TypeError $e) {
        echo "Error" . $e->getMessage();
    }
}
