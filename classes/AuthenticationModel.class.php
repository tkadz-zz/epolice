...
class AuthenticationModel extends Dbh{
    ...
    protected function register($loginID, $name, $surname, $nationalID, $passwordRaw, $userRole, $phone){
        $defaultContr = new DefaultContr();
        $activeStatus =1; $secondName = ''; $sex = ''; $country=''; $email=''; $address=''; $dob=''; $accessLevel='';
        $defaultContr->addUser($nationalID, $name, $surname, $loginID, $userRole, $activeStatus, $secondName, $sex, $country, $phone, $email, $address, $dob, $passwordRaw);
    }
    ...
    protected function SigninUser2ndStep($loginID, $password){
        ...
        $indexUser = $this->isUser($id, $role);

        $tName = $indexUser[0]['name'];
        $tSurname = $indexUser[0]['surname'];
    ...
    protected function login($loginID){
        ...
        if($rowsUser[0]['role'] == 'admin'){
            $_SESSION['accessLevel'] = $userRows[0]['accessLevel'];
        }
